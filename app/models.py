...
class Cuidados(db.Model):
    ...
    nombrecuidador = db.Column('nombrecuidador', db.String(20), index=True, unique=True)
    ...
    def __init__(self, nombrecuidador, nombrecomidadura, nombrecomidablanda, pautasalimentacion, tipohigiene, pautashigiene):
        self.nombrecuidador = nombrecuidador
	self.nombrecomidadura = nombrecomidadura
	self.nombrecomidablanda = nombrecomidablanda
	self.pautasalimentacion = pautasalimentacion
	self.tipohigiene = tipohigiene
	self.pautashigiene = pautashigiene

    def __repr__(self):
        return '<Cuidados %r>' % (self.nombrecuidador)
...
class Veterinario(db.Model):
    ...
    def __init__(self, nombreveterinario, tlfnoveterinario, tlfnourgencias, direccionveterinario, emailveterinario):
	self.nombreveterinario = nombreveterinario
	self.tlfnoveterinario = tlfnoveterinario
	self.tlfnourgencias = tlfnourgencias
	self.direccionveterinario = direccionveterinario
	self.emailveterinario = emailveterinario
